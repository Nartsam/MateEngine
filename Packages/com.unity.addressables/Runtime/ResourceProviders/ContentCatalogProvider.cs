        ...
        internal class InternalOp
        {
            ...
            internal string DetermineIdToLoad(IResourceLocation location, IList<object> dependencyObjects, bool disableCatalogUpdateOnStart = false)
            {
                    ...
                    location.Dependencies.Count == (int)DependencyHashIndex.Count)
                {
                    ...
                    {
#if ENABLE_CACHING
                        if (!string.IsNullOrEmpty(m_LocalHashValue) && !m_Retried && UnityEngine.AddressableAssets.AddressablesPortablePaths.HasCacheDataFolder) //cache exists and not forcing a retry state
                        {
...
#if ENABLE_CACHING
                            if (UnityEngine.AddressableAssets.AddressablesPortablePaths.HasCacheDataFolder)
...
#if ENABLE_CACHING
                    else if (string.IsNullOrEmpty(m_LocalDataPath) && !UnityEngine.AddressableAssets.AddressablesPortablePaths.HasCacheDataFolder)
                    {
                        Addressables.LogWarning($"Did not save cached content catalog because the portable Addressables cache path is empty.");
                    }
#endif
