...
namespace UnityEngine.ResourceManagement.ResourceProviders
{
    ...
    public class AssetBundleResource : IAssetBundleResource, IUpdateReceiver
    {
        ...
        internal bool CanUseUnityCache()
        {
#if !ENABLE_CACHING
            return false;
#else
            if (m_Options == null || string.IsNullOrEmpty(m_Options.Hash))
                return false;
            ...
            // MateEngine portable build: querying UnityEngine.Caching for local StreamingAssets bundles
            // initializes Unity's default cache location on Windows and creates a LocalLow directory.
            return ResourceManagerConfig.IsPathRemote(path);
#endif
        }
        ...
        internal UnityWebRequest CreateWebRequest(string url)
        {
            ...
            if (!string.IsNullOrEmpty(m_Options.Hash) && CanUseUnityCache())
            {
