...
| ADR-0009 | 2026-06-20 | 通用 VRM + App 内置可切换渲染风格（含仿 HoYo） | Retired → ADR-0010 |
| ADR-0010 | 2026-06-21 | VRM+RenderStyle 弯路复盘：.me 回归为主路径 | Accepted |
...
> 注：本 ADR 的"用 UTS2 映射 `.blend` 预设并烘进 `.me`"这一**渲染与输出**路线，经多轮 App 验收发现 UTS2 无法匹配 HoYo 多通道贴图语义（详见 `PROGRESS.md` M4 踩坑表），曾尝试由 **ADR-0009**（通用 VRM + App 内置可切换渲染风格，已于 2026-06-21 归档——见 ADR-0010）取代。前端（PMX 解析、Humanoid、物理检测、BlendShape）与离线管线框架仍有效并被复用；`.me` 确认为唯一生产路径。
...
状态：**Retired**（N0-N3 实现后实测不可行，详见 ADR-0010）
...
**事后注(2026-06-21)**：上述风险全部命中，N0-N3 实现完成后实测不可行，已回退到 .me 主路径。详见下方 ADR-0010。

---

## ADR-0010: VRM+RenderStyle 弯路复盘：.me 回归为主路径

**状态**：Accepted　｜　**日期**：2026-06-21

### 背景

ADR-0009 提出"通用 VRM + App 内置可切换渲染风格"架构，旨在用 vendored HoyoToon shader 在运行时逼近 HI3 游戏观感，同时保持 VRM 的通用性。该设计从 N0 到 N3 全部实现并通过编译/绑定验证，但在真实模型（丽塔 PMX→VRM）上实测失败。
