...
| ADR-0008 | 2026-06-20 | PMX 模型离线导入管线（Editor → `.me`） | Proposed |
...
- PMX 刚体/关节启发式转换为 DynamicBone 链（工程已内置 `Assets/DynamicBone`），近似 MMD 物理。
- 渲染：Blender 无头（`blender --background --python`）提取 `.blend` 预设每材质节点参数导出 JSON，自动映射到 UTS2/lilToon；脸部 SDF 与全局后处理对齐预设。无预设模型套用通用 NPR 基准。
- 终态为一键 batch 流水线：`Unity -batchmode -quit -executeMethod MEPmxPipeline.Build -pmx <path> -preset <path> -out <name>`，产物 `.me` 经现有加载路径载入。
...
- 物理→DynamicBone、预设→UTS2/lilToon 为有损近似，可能需逐模型手工微调；视觉/动作保真度需人工验收。
- `.me` 用 AssetBundle 打包 UTS2/lilToon 着色器时需验证着色器变体剥离（避免粉红），实施阶段复核。
- 按里程碑 M1–M5 推进，每步可独立验证。
