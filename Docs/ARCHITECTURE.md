...
PMX 管线是 Editor-only 预处理流程，目标是把 PMX 模型、MMD 物理和 Blender NPR 渲染预设转换为 Humanoid 模型，打包为 `.me` 格式（UTS2 材质 + DynamicBone 物理）。操作流程见 `PROGRESS.md` §正在处理（M1-M5 阶段）及 M4 踩坑表。注意：曾探索的 VRM+RenderStyle 路径（ADR-0009）已于 2026-06-21 归档（复盘见 `Docs/DECISIONS_RECORD.md` ADR-0010）。
