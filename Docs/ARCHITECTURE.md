...
PMX 管线是 Editor-only 预处理流程，目标是把 PMX 模型、MMD 物理和 Blender NPR 渲染预设转换为 Humanoid 模型。端到端操作流程（分阶段 runbook + 人工介入检查点 + 踩坑沉淀）见 `Docs/PMX_TO_VRM.md`；产物正从 `.me` 转向通用 VRM（`Docs/RENDER_STYLE_DESIGN.md`，ADR-0009）。
