...
> **⚠️ ARCHIVED (2026-06-21)** — 本设计实现至 N3 阶段后经实测证实不可行，已回退。
> 详见 `Docs/DECISIONS_RECORD.md` **ADR-0010** 获取复盘。
> 根本原因：① HoyoToon shader 在本项目 Built-in RP 环境下渲染异常（超亮/惨白、多轮调参无效）；
> ② VRM SpringBone 与 DynamicBone 底层物理算法不同，无法通过参数复刻 .me 的物理效果。
> 当前主路径：`.me` 管线（UTS2 + DynamicBone）。
>
> 状态：**已归档**　｜　最后更新：2026-06-21
